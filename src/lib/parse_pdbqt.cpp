...
public:
    ...
    struct node_t {
                    ...
                    const vec& frame_origin, bool skip_recenter) // ADDED param
        {
            ...
            vec relative_coords;
            if(skip_recenter) {
                // preserve absolute coords
                relative_coords = a.coords; 
            } 
            else {
                // original Vina code
        ...
        {
            VINA_FOR_IN(i, ps)
                ps[i].insert_immobile(nr, c, frame_origin, skip_recenter); // ADDED
        }
    };
                         ...
                         const vec& frame_origin, bool skip_recenter) // ADDED
    {
        if(!atoms.empty()) {
            ...
            // pass skip_recenter into node's insert(...) 
            atoms[immobile_atom.get()].insert(nr, c, frame_origin, skip_recenter);
        }
    }

    // original function without skip_recenter, if needed for flex
    void insert_immobile(non_rigid_parsed& nr, context& c, const vec& frame_origin) {
        if(!atoms.empty()) {
            ...
            // old approach (no skip param):
            atoms[immobile_atom.get()].insert(nr, c, frame_origin, false /* default? */);
        }
    }
...
void postprocess_branch(non_rigid_parsed& nr, parsing_struct& p, context& c, B& b, bool skip_recenter) {
    ...
    VINA_FOR_IN(i, p.atoms) {  // postprocess atoms into 'b.node'
        ...
        if (p.immobile_atom && i == p.immobile_atom.get()) {
            ...
            p_node.insert(nr, c, b.node.get_origin(), skip_recenter);
        p_node.insert_immobiles(nr, c, b.node.get_origin(), skip_recenter);
    }
    ...
    VINA_FOR_IN(i, p.atoms) {  // postprocess children
        ...
        VINA_FOR_IN(j, p_node.ps) {
            ...
            if (!ps.essentially_empty()) {  // immobile already inserted // FIXME ?!
                ...
                postprocess_branch(nr, ps, c, b.children.back(), skip_recenter);
            }
        }
    }
...
void postprocess_ligand(non_rigid_parsed& nr, parsing_struct& p, context& c, unsigned torsdof, bool skip_recenter) {
    ...
    postprocess_branch(nr, p, c, nr.ligands.back(), skip_recenter);
    ...
    VINA_FOR_IN(i, p.atoms) {  // iterate over "root" of a "residue"
        ...
        VINA_FOR_IN(j, p_node.ps) {
            ...
            if (!ps.essentially_empty()) {  // immobile atom already inserted // FIXME ?!
                ...
                postprocess_branch(nr, ps, c, nr.flex.back(), false);
            }
        }
    }
    nr_update_matrixes(nr);  // FIXME ?
...
void parse_pdbqt_ligand(std::istream& in, non_rigid_parsed& nr, context& c, bool skip_recenter = false) {
    parsing_struct p;
    ...
    try {
        postprocess_ligand(
            nr, p, c, unsigned(torsdof.get()), skip_recenter);  // bizarre size_t -> unsigned compiler complaint
    } catch (int e) {
        if (e == 1) {
...
void parse_pdbqt_ligand(const path& name, non_rigid_parsed& nr, context& c, bool keep_H = true, bool skip_recenter = false) {
    ifile in(name);
    parsing_struct p;
    boost::optional<unsigned> torsdof;
...
// SDF parsing removed; keeping old code under a renamed, unused symbol
#if 0
static void parse_sdf_aux_removed(std::istream& in, parsing_struct& new_p, parsing_struct& p, context& c,
...
model parse_ligand_pdbqt_from_file(const std::string& name,
                                   atom_type::t atype,
                                   bool keep_H,
                                   bool skip_recenter) {  // can throw parse_error
    non_rigid_parsed nrp;
    context c;

    try {
        parse_pdbqt_ligand(make_path(name), nrp, c, keep_H, skip_recenter);
...
model parse_ligand_from_file_no_failure(const std::string& name, atom_type::t atype,
    ...
    if (name.size() >= 5 && strcmp("pdbqt", name.substr(name.length() - 5, 5).c_str()) == 0) {
...
model parse_ligand_pdbqt_from_file_no_failure(const std::string& name, atom_type::t atype,
    ...
    if (tmp.m.ligands.count_torsions()[0] > DefaultConfig::MAX_NUM_OF_LIG_TORSION) {
    ...
    if (tmp.m.atoms.size() > DefaultConfig::MAX_NUM_OF_ATOMS) {
